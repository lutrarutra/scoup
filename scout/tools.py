...
def to_df(adata: sc.AnnData, index_col=None) -> None:
    if index_col:
        return pd.DataFrame(adata.X, index=adata.obs[index_col], columns=adata.var.index)
...
def annotate(adata: sc.AnnData, annotation_path: str, annotation_cols: list, barcode_col=0, header="infer") -> None:
    ...
    if type(barcode_col) == int:
        annotation = pd.read_csv(annotation_path, sep=sep, index_col=barcode_col, header=header)
...
def calculate_proportions(adata, groupby, clusterby):
    portions = pd.DataFrame(index=adata.obs[groupby].cat.categories, columns=adata.obs[clusterby].cat.categories)
    for group in adata.obs[groupby].cat.categories:
        ...
        for cluster in adata.obs[clusterby].cat.categories:
            portions.loc[group, cluster] = group_adata[group_adata.obs[clusterby] == cluster].obs.shape[0] / group_adata.obs.shape[0]
...
def correlation_matrix(adata, identify_by=None):
    if identify_by:
        return pd.DataFrame(np.corrcoef(adata.X), index=adata.obs.index + "_" + adata.obs[identify_by], columns=adata.obs.index + "_" + adata.obs[identify_by])
    return pd.DataFrame(np.corrcoef(adata.X), index=adata.obs.index, columns=adata.obs.index)
